> **SUPERSEDED (2026-08-19)** by `docs/frame-lattice-design-bible.md`. A frame is
> a cell of a fixed lattice now, so re-entry is a lookup rather than a
> nearest-neighbour decision: `REUSE_RADIUS`, `findChild`-by-distance and sibling
> spawning are DELETED, not tuned, and the `2~1` sibling id form no longer exists
> (ids are cell paths like `0/12,-3`). Read for the K-groups reasoning that led
> there; do not read it as a description of the code.
